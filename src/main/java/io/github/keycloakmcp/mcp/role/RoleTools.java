package io.github.keycloakmcp.mcp.role;
...
public class RoleTools {
    ...
    @Tool(name = "keycloak_list_roles", description = "List realm roles on a registered target")
    public List<RoleSummary> keycloakListRoles(
        ...
        return invoke(
                ...
                targetId,
                realm,
                () -> roleService.listRoles(targetId, realm, first, max));
    }

    @Tool(name = "keycloak_get_role", description = "Get a realm role by name on a registered target")
    public RoleDetails keycloakGetRole(
            @ToolArg(description = TARGET_ID_HINT) String targetId,
            @ToolArg(description = "Realm name") String realm,
            @ToolArg(description = "Role name") String roleName) {
        return invoke("keycloak_get_role", targetId, realm, () -> roleService.getRole(targetId, realm, roleName));
    }
