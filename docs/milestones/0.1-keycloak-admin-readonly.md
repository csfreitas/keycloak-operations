...
| | |
|--|--|
| Version | `0.1.0` (`pom.xml` at commit `198c237`) |
| Commit | `198c237` — *Initial commit: Keycloak/RHBK Operations Platform backend.* |
...
**Git note:** Roadmap milestones **0.1–0.3** were delivered together in a single initial commit (`198c237`) under artifact version **0.1.0**. There are no separate Git tags/commits for 0.1 vs 0.2 vs 0.3. This document isolates the **admin read-only MCP foundation** that that commit introduced.

**Tracking note:** An unanchored `.gitignore` entry `target/` initially excluded the Java package `io.github.keycloakmcp.target`. Those sources were restored in `5f9a1b7` (after 0.4). Multi-target types were intended from the start (CHANGELOG / message) but were not present in the Git tree until that fix.
...
Delivered in `198c237` (admin / MCP slice):
...
- Target package tracking gap until `5f9a1b7` (see Git note)
