...
| Commit | `4d01a9a` — *Add OpenShift/Kubernetes discovery and infrastructure inventory (0.4).* |
| Follow-up | `5f9a1b7` — restore `io.github.keycloakmcp.target` package tracking (gitignore fix) |
| Build / tests | CHANGELOG/`4d01a9a` added Fabric8 mock isolation + inventory tests and opt-in Keycloak ITs. Exact unit count at that commit not recorded in docs. |
...
Verified in `4d01a9a` (+ docs/CHANGELOG):
...
- [x] Target package tracked (`5f9a1b7`)
