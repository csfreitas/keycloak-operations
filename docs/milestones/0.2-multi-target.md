...
| | |
|--|--|
| Version | Shipped inside `0.1.0` initial tree (`198c237`); SemVer did not bump to `0.2.x` |
| Commit | Design/content intended in `198c237`; **Target package sources first tracked in** `5f9a1b7` (*Fix gitignore so the Java target package is tracked.*) |
...
**Git note:** There is **no** dedicated `0.2` commit. Multi-target was part of the initial platform design. Because `.gitignore` used unanchored `target/`, `io.github.keycloakmcp.target` sources were missing from `198c237` and `4d01a9a` on Git until `5f9a1b7`.
...
- [x] Target MCP tools
- [x] Isolation tests
- [x] Target package tracked in Git (`5f9a1b7`)
...
- Git history for 0.2 is not a clean SemVer tag; reconstruct from docs + `5f9a1b7` + initial CHANGELOG
